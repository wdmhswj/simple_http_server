...
#define SHS_LOG_LEVEL(logger, level) \
    if(logger->getLevel() <= level) \
        (shs::LogEventWrap(shs::LogEvent::ptr(new shs::LogEvent(logger, level, __FILE__, __LINE__, 0, shs::GetThreadIdBySyscall(), shs::GetFiberId(), time(0), shs::Thread_posix::GetName()))).getSS())
